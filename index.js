...
// Start Telegram Pairing Bot (same process)
try {
  const { startTelegramBot } = require('./telegram');
  startTelegramBot().catch(e => console.error('Telegram start error:', e.message));
} catch (e) {
  console.error('Telegram module error:', e.message);
}
...
async function startBot() {
  ...
  // Check if sessionID is provided and process RehanBot! format session
  if (config.sessionID && config.sessionID.startsWith('RehanBot!')) {
    try {
      const [header, b64data] = config.sessionID.split('!');

      if (header !== 'RehanBot' || !b64data) {
        throw new Error("❌ Invalid session format. Expected 'RehanBot!.....'");
      }
      ...
      console.log('📡 Session : 🔑 Retrieved from RehanBot Session');

    } catch (e) {
      console.error('📡 Session : ❌ Error processing RehanBot session:', e.message);
